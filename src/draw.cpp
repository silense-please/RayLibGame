...
void draw_player_animation(Player player, Animation &animation){
    ...
    float frame_width = (float)animation.texture.width / frames;
    float frame_height = animation.texture.height;

    Rectangle run_frame_rec = {(float)animation.current_frame * frame_width, 0, frame_width * flip, frame_height};
...
void draw_animation(Animation &animation, int frames, Vector2 dest){
    //int frames = animation.texture.width / TILESIZE;
    float frame_width = (float)animation.texture.width / frames;
    float frame_height = animation.texture.height;

    Rectangle run_frame_rec = {(float)animation.current_frame * frame_width, 0, frame_width, frame_height};
    DrawTextureRec(animation.texture, run_frame_rec, {(float) dest.x - frame_width / 2, (float) dest.y - frame_height / 2}, WHITE);
    animation.timer += delta_time;

    if(_draw_debug_info){ // Visualising to see if animations are working properly
        DrawText(TextFormat("%d", animation.current_frame ), dest.x, dest.y - 20, 5, animation.current_frame%2 == 0?LIME:BLUE);
        DrawText(TextFormat("timer: %f", animation.timer), dest.x, dest.y - 40, 20, animation.current_frame%2 == 0?LIME:BLUE);
    }
...
// Reset animation if it's finished. @ DELETE THIS?
